...
let add_type_variable env t (k, v) =
  { env with type_info = Env.add env.type_info t (k, v, ref Abstract) }
...
let fresh_types xs ctx =
  ...
  (fresh_vars, ctx (change_arterm_vars fresh_assoc))

let fresh_scheme xs t =
  fresh_types xs (fun f -> f t)
...
let fresh_product_of_label pos env l =
  try
    let (vs, rty, ltys) =
